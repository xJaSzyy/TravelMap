...
function PlacePopup({ place }) {
    ...
    return (
        <div className="popup-content">
            ...
            <div className={styles.popupDescription}>{place.description}</div>

            <div className={styles.popupInfo}>
                {place.rating && (
                    <div className={styles.popupRating}>⭐ {place.rating.toFixed(1)}</div>
                )}

                {place.hours && <div className={styles.popupHours}>🕒 {place.hours}</div>}
            </div>
            ...
            <div className={styles.popupButtons}>
                ...
                <button
                    id={`route-btn-${place.id}`}
                    className={`${styles.mapIcon} ${styles.routeButton}`}
                >
